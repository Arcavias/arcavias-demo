...
class TestHelper
{
	...
	private static function _getArcavias()
	{
		if( !isset( self::$_arcavias ) )
		{
			...
			require_once $base . $ds . 'vendor' . $ds . 'arcavias' . $ds . 'arcavias-core' . $ds . 'Arcavias.php';
			self::$_arcavias = new Arcavias( array( $base . $ds . 'ext' ), false );
		}
	...
	private static function _createContext( $site )
	{
		...
		$paths[] = dirname( __DIR__ ) . DIRECTORY_SEPARATOR . 'config';
		...
		$locale = $localeManager->bootstrap( $site, 'en', '', false );
